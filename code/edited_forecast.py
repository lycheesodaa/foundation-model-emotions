...
class MoiraiForecast(L.LightningModule):
    ...
    # new method to format the distr.mean
    def _format_mean(
            self,
            patch_size: int,
            mean: Float[torch.Tensor, "batch combine_seq patch"],
            target_dim: int,
    ) -> Float[torch.Tensor, "batch future_time *tgt"]:
        start = target_dim * self.context_token_length(patch_size)
        end = start + target_dim * self.prediction_token_length(patch_size)
        mean = mean[..., start:end, :patch_size]
        mean = rearrange(
            mean,
            "... (dim seq) patch -> ... (seq patch) dim",
            dim=target_dim,
        )[..., :self.hparams.prediction_length, :]
